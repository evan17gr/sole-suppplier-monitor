...
import { getProxyList } from './proxies/proxyList';
...
const getData = async () => {
	const userAgent = randomUserAgent.getRandom(ua => {
		return (
			(ua.browserName === 'Chrome' || ua.browserName === 'Safari') &&
	...
	const proxy: Proxy = await getProxyList();
	console.log(proxy);
	...
	try {
		...
		const formattedShoes: Shoes[] = shoes.map((shoe: Shoe) => {
			return {
				...
				productDate: shoe?.date?.released,
			};
		});
